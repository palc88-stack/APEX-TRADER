...
class ExchangeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if invalid_exchanges:
            raise ValueError(
                ...
                "Supported values are: binance, bybit."
            )
    ...
    def enabled_exchanges(self) -> List[str]:
        ...
        for exchange_name in self.trading_exchanges:
            if self.credentials_available(exchange_name):
                ...
                logger.warning(
                    "⚠️ %s مطلوب لكنه لا يملك مفاتيح API كاملة",
                    exchange_name,
                )
    ...
    def primary_exchange(self) -> str:
        ...
        if not enabled:
            raise RuntimeError(
                "لا توجد منصة تحتوي على API credentials كاملة"
            )
...
class DatabaseConfig:
    ...
    def validate(self) -> bool:
        ...
        if not has_supabase:
            logger.warning(
                "⚠️ Supabase غير مضبوط؛ "
                "لن يمكن استعادة الصفقات من قاعدة البيانات"
            )
