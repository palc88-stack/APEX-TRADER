...
class ApexTrader:
    ...
    async def _review_open_positions(self, balance: float) -> None:
        for position in self.position_manager.get_all_positions():
            try:
                ...
                elif action and action['action'] == 'partial_close':
                    await self._partial_close_position(
                        position, action['price'], action['percentage'], balance
                    )
            except Exception as error:
        ...
        for symbol in config.trading.symbols:
            try:
                ...
                signal = self.signal_engine.analyze(
                    ...
                    await self.exchange.get_orderbook(symbol)
                )
                if signal.is_valid:
    ...
    async def _execute_signal(self, signal: TradeSignal, balance: float) -> None:
        ...
        targets = self.fee_calculator.adjust_targets(
            signal.entry_price, signal.direction.value.lower(),
            config.risk.default_sl_pct, config.risk.tp1_pct,
            config.risk.tp2_pct, position_value
        )
        try:
            order = await self.exchange.place_order(
                signal.symbol, signal.direction.value.lower(),
                risk_check.adjusted_size, risk_check.adjusted_leverage,
                targets['stop_loss'], targets['tp1']
            )
            if not order:
                return
            position = Position(
                id=str(uuid.uuid4())[:8], symbol=signal.symbol,
    ...
    async def _apply_compounding(self, pnl: float, balance: float) -> None:
        if pnl > 0:
            ...
            await self.state_manager.record_compounding(pnl - (pnl - compound), pnl - compound)
